...
function endErrorTime(){
      setTimeout( ()=> errorBox.classList.remove('show-error'),6000)
}
...
forms.forEach((form,i) => form.addEventListener('submit', (e) =>{
    e.preventDefault();
    switch(i){
        case 0 : (async function fillArticle(id){
            let init = {
                       method:'POST',
                       headers:{
        ...
        case 1:(async function fillArticle(id){
            let init = {
                       method:'POST',
                       headers:{
                           "content-type":'application/json'
            ...
            if(reponse.status == 200)
            {
                window.location.herf = 'http://127.0.0.1:5500/html/home.html';
                alert('bienvenu '+ firstName.value)
                return false;
            }
            else
            {
